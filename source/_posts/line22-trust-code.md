...
`key` 값을 유출하였으므로 셸코드를 암호화하여 전송하면 되는데, `\x0f` 와 `\x05` 를 사용하면 실패하는 조건이 있습니다. 따라서 `syscall` 와 `sysenter` 인스트럭션을 통해 시스템 콜을 호출할 수 없습니다. 그런데 코드가 복사 후 실행되는 영역은 쓰기와 실행이 모두 가능한 rwx 페이지입니다. 따라서 런타임에 동적으로 셸코드의 내용을 수정하는 코드를 추가하면 `syscall` 인스트럭션을 실행할 수 있습니다.
